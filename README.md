...
`InternalNode.Learn()` method calls its children's `Learn()` method recursively, so you have to call `Learn()` method only once. After learning, you will obtain the assignments for each data point to the clusters, which were called *temporal groups* in [the original paper](http://alpha.tmit.bme.hu/speech/docs/education/02_DileepThesis.pdf) by Dileep George.
